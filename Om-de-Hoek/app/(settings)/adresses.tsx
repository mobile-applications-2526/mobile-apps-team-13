...
import AddNewEntityCard from "@/components/card/AddNewEntityCard";
...
const AddressSettings = () => {
    ...
    const saveNew = async (updatedAddress: Address) => {
        ...
        try {
            const createdAddress = await addressService.RegisterAddress(command, token!);
            ...
            if (e instanceof UnauthorizedError) {
                console.warn("Error creating address: token expired, refreshing tokens");
                await refreshTokens()
                const waiter = new Promise(resolve => setTimeout(resolve, 1000));
                await waiter;
                const createdAddress = await addressService.RegisterAddress(command, token!);
        ...
        >
            ...
            {addresses.map((address) => (
                <AdressCard
                    ...
                    onDelete={() => {handleDeleteAddress(address.adresId).then();}}
                    onCancel={() => {
                        setOpenedAddresss(undefined);
                    }}
                />
