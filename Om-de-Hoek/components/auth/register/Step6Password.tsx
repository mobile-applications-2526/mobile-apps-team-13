...
export const Step6Password = ({ onNext, onChange }: Props) => {
    ...
    useEffect(() => {
        ...
        const overallValid = minLengthValid && numberValid && uppercaseValid && specialCharValid;
        setIsValid(overallValid);

        const score = [minLengthValid, numberValid, uppercaseValid, specialCharValid].filter(Boolean).length;
        setStrength(score);
        ...
        <ScrollView
            ...
            <View className='my-4'>
                <View className="flex-row mb-2">
                    ...
                    <Text className={`font-comfortaa-bold ${strengthFeedback.color}`}>{strengthFeedback.text}</Text>
                </View>
